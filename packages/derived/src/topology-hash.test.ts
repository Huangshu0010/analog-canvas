...
function baseDoc(
  overrides: Partial<CircuitProject["documents"][number]>,
): CircuitProject["documents"][number] {
  return {
    id: "doc",
    name: "Doc",
    revision: 0,
  ...
  it("changes when a terminal is added to a Net", () => {
    const before = project([
      baseDoc({
        ...
        nets: [{ id: "n1", scope: "local", terminals: [], ports: [] }],
      }),
    ]);
    const after = project([
      baseDoc({
    ...
    expect(electricalTopologyHash(before)).not.toBe(
      electricalTopologyHash(after),
    );
  });
