...
export function insertRouteSegmentJog(
  polyline: RouteEditPath,
  segmentIndex: number,
  offset: number,
): { waypoints: Point[]; segmentModes: SegmentMode[] } {
  ...
  if (!Number.isInteger(offset) || offset === 0) {
    throw new Error("Route jog offset must be a non-zero integer");
  }
  const mode = polyline.segmentModes[segmentIndex]!;
  if (mode === "locked" || mode === "trunk") {
    throw new Error("Route segment is protected");
  }
  ...
  if (!horizontal && !vertical) {
    throw new Error("Route segment must be orthogonal");
  }
  const buildJog = (signedOffset: number) => {
    const points = polyline.points.map((point) => ({ ...point }));
    const modes = [...polyline.segmentModes];
    const first = horizontal
      ? { x: from.x, y: from.y + signedOffset }
      : { x: from.x + signedOffset, y: from.y };
    const second = horizontal
      ? { x: to.x, y: to.y + signedOffset }
      : { x: to.x + signedOffset, y: to.y };
    points.splice(segmentIndex + 1, 0, first, second);
    modes.splice(segmentIndex, 1, mode, mode, mode);
    return normalizeRouteGeometry(points, modes);
  };
  // At an existing corner, one perpendicular direction can merge with its
  // neighbor during normalization. Prefer the other direction so this command
  // always creates a reversible, explicit three-segment jog.
  const normalized = [offset, -offset]
    .map(buildJog)
    .find(
      (candidate) => candidate.points.length === polyline.points.length + 2,
    );
  if (!normalized) {
    throw new Error("Route segment has no room for an isolated orthogonal jog");
  }
  return {
    waypoints: normalized.points.slice(1, -1),
    segmentModes: normalized.segmentModes,
...
/** Straightens the protected-free jog whose middle segment is selected. */
export function removeRouteSegmentJog(
  polyline: RouteEditPath,
  segmentIndex: number,
): { waypoints: Point[]; segmentModes: SegmentMode[] } {
  if (segmentIndex <= 0 || segmentIndex >= polyline.points.length - 2) {
    throw new Error("Select the middle segment of a route jog to remove it");
  }
  const affectedModes = polyline.segmentModes.slice(
    segmentIndex - 1,
    segmentIndex + 2,
  );
  if (affectedModes.some((mode) => mode === "locked" || mode === "trunk")) {
    throw new Error("Route jog is protected");
  }
  const points = polyline.points.map((point) => ({ ...point }));
  const modes = [...polyline.segmentModes];
  points.splice(segmentIndex, 2);
  modes.splice(segmentIndex - 1, 3, affectedModes.reduce(strongerMode));
  if (!isOrthogonal(points)) {
    throw new Error("Selected segment is not a removable orthogonal jog");
  }
  const normalized = normalizeRouteGeometry(points, modes);
  return {
    waypoints: normalized.points.slice(1, -1),
