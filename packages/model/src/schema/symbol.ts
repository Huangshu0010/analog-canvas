...
/**
 * Canonical connection grid for symbol pin anchors. Persisted symbol
 * artwork is validated against this grid at the persistence boundary.
 */
...
/**
 * ADR 0047: project-authority record for one user-defined symbol. The `id`
 * is the stable definition identity project references point through; the
 * embedded `symbol` is the validated artwork exactly as the Symbol DSL
 * defines it. The runtime symbol ID is derived from `id` in a dedicated
 * namespace, so embedded artwork can never shadow a catalog asset.
 */
export const CustomSymbolDefinitionSchema = z.strictObject({
  id: StableIdSchema,
...
export type CustomSymbolDefinition = z.infer<
  typeof CustomSymbolDefinitionSchema
>;
