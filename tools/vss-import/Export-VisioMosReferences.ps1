...
if ([string]::IsNullOrWhiteSpace($OutputDirectory)) {
    $OutputDirectory = Join-Path $PSScriptRoot "..\..\fixtures\visual-reference\visio-mos"
}
...
$temporaryDirectory = [IO.Path]::GetFullPath(
    (Join-Path $temporaryRoot "icm-visio-mos-$([Guid]::NewGuid().ToString('N'))")
)
...
try {
    foreach ($entry in $masters.GetEnumerator()) {
        ...
        if ($Check) {
            if (-not (Test-Path -LiteralPath $targetPath)) {
                throw "Missing Visio MOS reference: $targetPath"
            }
            $expected = [IO.File]::ReadAllText($targetPath).Replace("`r`n", "`n")
            if ($expected -ne $normalized) {
                throw "Visio MOS reference is stale: $fileName"
            }
        }
        else {
    ...
    Write-Output "$action $($masters.Count) isolated Visio MOS references"
}
finally {
    if (
        $temporaryDirectory.StartsWith($temporaryRoot, [StringComparison]::OrdinalIgnoreCase) -and
        [IO.Path]::GetFileName($temporaryDirectory).StartsWith("icm-visio-mos-") -and
