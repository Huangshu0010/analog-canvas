...
test("exports deterministic SPICE and Spectre design netlists", async ({
  page,
}) => {
  const project = createEmptyProject("netlist-export", "Netlist Export");
  const document = project.documents[0]!;
  document.instances.push({
    id: "resistor-1",
    symbolId: "resistor",
  ...
  const child = createEmptyDocument("document-child", "Leaf");
  child.netlist = { name: "leaf", portOrder: [] };
  ...
  expect(spectre).toBe(
    "// Generated by Interactive Circuit Maker netlist-export/1.0\n" +
      ...
      "ends leaf\n\n" +
      "subckt Main\n" +
      "R1 (VDD 0) resistor r=10k\n" +
      "X1 () leaf\n" +
      "ends Main\n",
  );
});

test("blocks netlist download and reports missing electrical facts", async ({
  page,
}) => {
  const project = createEmptyProject("netlist-errors", "Netlist Errors");
  project.documents[0]!.instances.push({
    id: "mos-1",
    symbolId: "nmos",
    placement: null,
    properties: {},
    netlist: { reference: "M1", parameters: { w: "2u", l: "60n" } },
  });

  await page.goto("/");
  await openMenu(page, "File");
  await page.getByTestId("project-file").setInputFiles({
    name: "invalid-netlist.icproj.json",
    mimeType: "application/json",
    buffer: Buffer.from(JSON.stringify(project)),
  });
  ...
  await expect(page.getByTestId("status")).toContainText(
    "Netlist export blocked",
  );
  await openMenu(page, "File");
  await expect(
    page.getByRole("region", { name: "Netlist export diagnostics" }),
  ).toContainText("MISSING_MODEL_TARGET");
  await page.getByRole("button", { name: /MISSING_MODEL_TARGET/u }).click();
  await expect(page.getByTestId("status")).toContainText(
