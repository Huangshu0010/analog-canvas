...
Portable Projects use canonical schema-24 `.icproj.json`. The current-only
...
platform supports it. Schema 23 upgrades to 24 at ingestion; serialization
always writes schema 24. Older schemas are rejected outside the rolling
current-and-previous compatibility window.

Recovery state is a non-authoritative browser safety copy. It may restore a
complete schema-24 Project or a schema-23 record that validates after the
