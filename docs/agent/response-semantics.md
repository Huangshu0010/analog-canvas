...
| Visual diagnostic | A measurable issue or observation exists at a known revision/location. | Errors block; warnings require judgment; info is evidence. | Repair the smallest responsible area or record why intentional. |
...
| `VISUAL_SYMBOL_OVERLAP` | Warning: instance silhouettes overlap. | Separate unlocked objects while preserving functional grouping. |
| `VISUAL_LABEL_OVERLAP` | Warning: label bounds overlap another label/symbol. | Move text locally before moving electrical structure. |
...
| `VISUAL_WIRE_THROUGH_SYMBOL` | Warning: Route crosses a non-endpoint instance silhouette. | Change graph/placement, not stroke style. |
| `VISUAL_ROUTE_OVERLAP` | Warning: same-Net Routes duplicate a collinear span. | Collapse duplicate geometry or use a clearer branch/label. |
...
Diagnostics do not currently identify every confusing small box, duplicated
