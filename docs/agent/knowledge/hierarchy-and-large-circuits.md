...
candidate child, read its complete Snapshot and relate its ports back to the
parent instance pins. A repeated child instance is one definition with several
contexts; do not clone or flatten it merely to make reasoning easier.
...
- Keep stable repeated cells hierarchical when their ports state the function
  clearly and the parent still exposes signal order and weighting.
...
- In the parent render, every child port must be visibly connected or locally
  identifiable. Repeated common connections may use one compact trunk/rail or a
...
Similar subcircuit names do not prove equivalent behavior. Check port order,
parameters, model variants, and external Net roles. Shared child definitions can
