...
| Resolved route geometry | Formal renderer, editor display/hit/marker, Agent reads, visual diagnostics and stretch inputs consume document-level geometry.  | Route-anchor/drafting compatibility reads and Edit Engine mutation/validation still retain the lower-level primitive. |
...
| Compatibility cleanup   | Consumer audit complete for migrated read paths; no blind deletion performed.                                                      | `routePolyline` remains the geometry derivation primitive and is still required by route-anchor/drafting compatibility plus Edit Engine transaction validation/mutation. |
